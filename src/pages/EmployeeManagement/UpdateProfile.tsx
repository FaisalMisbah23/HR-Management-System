...
const UpdateProfile = () => {
    const [data,setData]=useState({
        name:'',email:'',phoneNumber:'',department:'',jobtitle:'',jobcategory:'',gender:'',startdate:''})

    const {employeeId}=useParams()
    const fetchEmp = async () => {
        const res = await axios.get(`${import.meta.env.VITE_USER_URL}/auth/${employeeId}`,{withCredentials:true})
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.patch(
        `${import.meta.env.VITE_USER_URL}/auth/update/${employeeId}`,
        {
