...
import { Target } from "../../types/package";
  ...
  const fetchTargets = async () => {
    try {
      const res = await axios.get(
        `${import.meta.env.VITE_ADMIN_URL}/target/`,{withCredentials:true})        
        setData(res.data.data[0].targets)
    } catch (error) {
      console.log(error)
    }
  }
...
const [targetIdToDelete, setTargetToDelete] = useState(null);
...
useEffect(() => {
  if (isDelete && targetIdToDelete) {
    deleteEmp(targetIdToDelete);
  }
}, [isDelete, targetIdToDelete]);

const deleteEmp = async (targetId) => {
  if (!isDelete) return;
...
const initiateKpi = async (targetId) => {
  try {    
    const response = await axios.post(
      `${import.meta.env.VITE_ADMIN_URL}/target/addkpiscore/${targetId}`,
      {},
      { withCredentials: true }
    );
    toast.success(response.data.message);
  ...
  return (
          ...
          <tbody>
            {data && data.map((packageItem, key) => (
              <tr key={key}>
                ...
                <td className="border-b border-[#eee] py-5 px-4 dark:border-strokedark">
                  <p className="text-black dark:text-white">
                    {formatDate(packageItem.startdate)}/{formatDate(packageItem.enddate)}
                  </p>
                </td>
                <td className="border-b border-[#eee] py-5 px-4 dark:border-strokedark">
                  <div className="flex items-center space-x-3.5">
                    <button
                    onClick={()=>initiateKpi(packageItem._id)}
                    className="hover:text-primary">
                   {packageItem.marking?<svg xmlns="http://www.w3.org/2000/svg" width="22" height="22" viewBox="0 0 24 24">
	...
	<path fill="currentColor" d="M20 8h-5.612l1.123-3.367c.202-.608.1-1.282-.275-1.802S14.253 2 13.612 2H12c-.297 0-.578.132-.769.36L6.531 8H4c-1.103 0-2 .897-2 2v9c0 1.103.897 2 2 2h13.307a2.01 2.01 0 0 0 1.873-1.298l2.757-7.351A1 1 0 0 0 22 12v-2c0-1.103-.897-2-2-2M4 10h2v9H4zm16 1.819L17.307 19H8V9.362L12.468 4h1.146l-1.562 4.683A.998.998 0 0 0 13 10h7z" />
                    ...
                    <Link to={`/admin/performancemanagement/target/${packageItem._id}`}>
                    <button 
                    className="hover:text-primary">
